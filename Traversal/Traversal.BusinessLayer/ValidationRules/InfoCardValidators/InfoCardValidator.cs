using FluentValidation;
using Traversal.DtoLayer.DTOS.InfoCardDtos;

namespace Traversal.BusinessLayer.ValidationRules.InfoCardValidators
{
    public class InfoCardValidator : AbstractValidator<CreateInfoCardDto>
    {
        public InfoCardValidator()
        {
            RuleFor(x => x.Title)
                .NotEmpty().WithMessage("Başlık boş bırakılamaz.")
                .MinimumLength(2).WithMessage("Başlık en az 2 karakter olmalıdır.")
                .MaximumLength(100).WithMessage("Başlık en fazla 100 karakter olabilir.");

            RuleFor(x => x.SubTitle)
                .NotEmpty().WithMessage("Alt başlık boş bırakılamaz.")
                .MaximumLength(50).WithMessage("Alt başlık en fazla 50 karakter olabilir.");

            RuleFor(x => x.Description)
                .NotEmpty().WithMessage("Açıklama boş bırakılamaz.")
                .MinimumLength(10).WithMessage("Açıklama en az 10 karakter olmalıdır.")
                .MaximumLength(500).WithMessage("Açıklama en fazla 500 karakter olabilir.");

            RuleFor(x => x.Image)
                .NotEmpty().WithMessage("Görsel alanı boş bırakılamaz.");
        }
    }
}